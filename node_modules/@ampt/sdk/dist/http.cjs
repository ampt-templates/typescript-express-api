...
var Http = class {
  ...
  use(prefixOrHandler, _handler) {
    ...
    if (!this.prefixes.has(eventPrefix)) {
      addEventListener(`http:any:${eventPrefix}*`, (event, eventContext) => {
        event.respondWith(this.handleEvent(event, eventContext));
      });
      this.prefixes.add(eventPrefix);
    }
  }
  ...
  async handleEvent(event, context) {
    ...
    for (const loader of this.loaders) {
      ...
      const _handler = handler(event.request, context);
      if (!_handler) {
        continue;
      }
    ...
    return notFoundResponses.shift() || new Response("Not Found", { status: 404 });
  }
  nodeAdapter(handler) {
    const options = {
      binary: void 0,
  ...
  on(eventName, relativePath) {
    if (eventName.toString() === "404") {
      addEventListener(`http:any:*`, (event) => {
        event.respondWith(
          new StreamingResponse(createReadableStream(relativePath), {
            headers: {
