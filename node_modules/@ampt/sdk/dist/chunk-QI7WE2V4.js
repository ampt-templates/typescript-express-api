  ...
  "../../node_modules/fast-xml-parser/src/xmlparser/DocTypeReader.js"(exports, module2) {
    function readDocType(xmlData, i4) {
      ...
      if (xmlData[i4 + 3] === "O" && xmlData[i4 + 4] === "C" && xmlData[i4 + 5] === "T" && xmlData[i4 + 6] === "Y" && xmlData[i4 + 7] === "P" && xmlData[i4 + 8] === "E") {
        ...
        for (; i4 < xmlData.length; i4++) {
          if (xmlData[i4] === "<") {
            if (hasBody && xmlData[i4 + 1] === "!" && xmlData[i4 + 2] === "E" && xmlData[i4 + 3] === "N" && xmlData[i4 + 4] === "T" && xmlData[i4 + 5] === "I" && xmlData[i4 + 6] === "T" && xmlData[i4 + 7] === "Y") {
              i4 += 7;
              entity = true;
            } else if (hasBody && xmlData[i4 + 1] === "!" && xmlData[i4 + 2] === "E" && xmlData[i4 + 3] === "L" && xmlData[i4 + 4] === "E" && xmlData[i4 + 5] === "M" && xmlData[i4 + 6] === "E" && xmlData[i4 + 7] === "N" && xmlData[i4 + 8] === "T") {
              i4 += 8;
            } else if (hasBody && xmlData[i4 + 1] === "!" && xmlData[i4 + 2] === "A" && xmlData[i4 + 3] === "T" && xmlData[i4 + 4] === "T" && xmlData[i4 + 5] === "L" && xmlData[i4 + 6] === "I" && xmlData[i4 + 7] === "S" && xmlData[i4 + 8] === "T") {
              i4 += 8;
            } else if (hasBody && xmlData[i4 + 1] === "!" && xmlData[i4 + 2] === "N" && xmlData[i4 + 3] === "O" && xmlData[i4 + 4] === "T" && xmlData[i4 + 5] === "A" && xmlData[i4 + 6] === "T" && xmlData[i4 + 7] === "I" && xmlData[i4 + 8] === "O" && xmlData[i4 + 9] === "N") {
              i4 += 9;
            } else if (
              //comment
              xmlData[i4 + 1] === "!" && xmlData[i4 + 2] === "-" && xmlData[i4 + 3] === "-"
            ) {
              comment = true;
            } else {
  ...
  if (isRetryStrategyV2(retryStrategy)) {
    ...
    while (true) {
      try {
        ...
        const retryErrorInfo = getRetyErrorInto(e4);
...
var decorateServiceException = (exception, additions = {}) => {
  Object.entries(additions).filter(([, v2]) => v2 !== void 0).forEach(([k4, v2]) => {
...
var fromEnv2 = (envVarSelector) => async () => {
  try {
    ...
    throw new CredentialsProviderError(e4.message || `Cannot load config from environment variables with getter: ${envVarSelector}`);
  }
};
...
var fromSharedConfigFiles = (configSelector, { preferredFile = "config", ...init } = {}) => async () => {
  ...
  try {
    ...
    throw new CredentialsProviderError(e4.message || `Cannot load config for profile ${profile} in SDK configuration files with getter: ${configSelector}`);
  }
};
  ...
  const whatwgURL = (() => {
    try {
      ...
      if (typeof value === "object" && "hostname" in value) {
        ...
        url.search = Object.entries(query).map(([k4, v2]) => `${k4}=${v2}`).join("&");
        return url;
      }
...
var resolveEndpoint = (ruleSetObject, options) => {
  ...
  const paramsWithDefault = Object.entries(parameters).filter(([, v2]) => v2.default != null).map(([k4, v2]) => [k4, v2.default]);
  if (paramsWithDefault.length > 0) {
    for (const [paramKey, paramDefaultValue] of paramsWithDefault) {
      endpointParams[paramKey] = endpointParams[paramKey] ?? paramDefaultValue;
    }
  }
  const requiredParams = Object.entries(parameters).filter(([, v2]) => v2.required).map(([k4]) => k4);
...
var resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, profile }) => {
  ...
  if (ssoSession) {
    try {
      ...
      throw new CredentialsProviderError(e4.message, SHOULD_FAIL_CREDENTIAL_CHAIN);
    }
  } else {
    try {
  ...
  try {
    ...
    throw CredentialsProviderError.from(e4, SHOULD_FAIL_CREDENTIAL_CHAIN);
  }
